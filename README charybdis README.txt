...
Six scripts were used to calculate and plot the main data from a neurosciene/linguistics experiment with the working title Charybdis. Together with the raw data available elsewhere, they should be sufficient to reproduce the plots and statistical results in the resulting paper. The scripts are:

1. charybdis preproc.txt <- run this first to preprocess the data
2. charybdis make study to select eye ICs <- guess what it does
3. charybdis erplab preprocessing scrip <- for the ERPs (figure 1)
4. charybdis erpimages <- ERPimages (figure 2)
5. charybdis quantification <- correlation and variance test (fig 3)
6. charybdis timtopo <- Butterfly plots (supp figs)
...
On any reasonably fast computer (double digit RAM and more cores, and GHz per core, than Kant had thumbs), the whole show should be done within less than a day.
...
The other three EEGLAB scripts can be run in any order.
...
"quantification" basically does the same as plot erpimages, but it doesn't give pretty images, but numbers and simple plots. I'm sorry that it's not especially ... good. It also includes a few plots and measures not included in the original analysis because I didn't sell them hard enough to my coauthors.
"timtopo" is used to make pretty butterfly plots (supplemental material).
